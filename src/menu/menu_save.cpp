...
// Helper functions for SF2000 save state paths
static void get_game_basename(const char *path, char *out, size_t size)
{
...
static void build_save_path(int slot, char *out, size_t size)
{
	char basename[64];
	extern char uae4all_image_file[];

	get_game_basename(uae4all_image_file, basename, sizeof(basename));
	snprintf(out, size, "/mnt/sda1/ROMS/save/%s_S%d.asf", basename, slot);
}
...
//				show_please_wait("Loading");
				{
				// Build proper SF2000 save path (slots 1-4 map to S1-S4)
				build_save_path(saveMenu_n_savestate + 1, savestate_filename, 100);
