...
char uae4all_image_file3[128] = { 0, };  // v025: DF2 support
char uae4all_image_file4[128] = { 0, };  // v025: DF3 support
...
// SE EJECUTA DESPUES DE INICIAR EL CORE 68k
    // v032: Support all 4 disk drives at startup
    strcpy(changed_df[0],uae4all_image_file);
    strcpy(changed_df[1],uae4all_image_file2);
    strcpy(changed_df[2],uae4all_image_file3);
    strcpy(changed_df[3],uae4all_image_file4);
...
//   vkbd_init();
   ...
   SDL_DC_EmulateKeyboard(SDL_FALSE);
    ...
    if (exitmode==1 || exitmode==2)
    {
	    ...
	    // v031: Add DF2 and DF3 support
	    if (strcmp(changed_df[2],uae4all_image_file3))
	    {
            	strcpy(changed_df[2],uae4all_image_file3);
	    	real_changed_df[2]=1;
	    }
	    if (strcmp(changed_df[3],uae4all_image_file4))
	    {
            	strcpy(changed_df[3],uae4all_image_file4);
	    	real_changed_df[3]=1;
	    }
...
#endif
	    changed_df[1][0]=0;
	    changed_df[2][0]=0;  // v031: clear DF2
	    changed_df[3][0]=0;  // v031: clear DF3
	    if (strcmp(changed_df[0],uae4all_image_file))
	    {
            	strcpy(changed_df[0],uae4all_image_file);
	    	real_changed_df[0]=1;
...
{
#ifdef __LIBRETRO__
	// SF2000/libretro: SDL not available, skip progress bar
	(void)per;
	(void)max;
	(void)case_title;
#else
