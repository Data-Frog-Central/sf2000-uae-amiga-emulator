...
#include "glob.h"
#else
/* SF2000 - no glob support on bare metal, provide stubs */
typedef struct {
    size_t gl_pathc;
    char **gl_pathv;
} glob_t;
...
char romfile_sd[32];
#endif
int real_changed_df[NUM_DRIVES]={0,0,0,0};
...
#ifdef DEBUG_DISK
*/
// v024: Removed 'static' so floppy_speed can be modified from core-mapper.cpp
int floppy_speed = NORMAL_FLOPPY_SPEED;
...
/* v055: Disk shuffler system - stores ALL detected disks (up to 10) for rotation */
#define MAX_MULTIDISK 10
char multidisk_paths[MAX_MULTIDISK][256];  // All detected disk paths
int multidisk_count = 0;                    // Total number of disks found
int multidisk_offset = 0;                   // Current rotation offset (which disk is in df0)
 ...
 * Detects numbered disk files (e.g., game1.adf, game2.adf) and loads them into multiple drives.
 * Disk 0 is treated as autosave disk and goes LAST.
 * Example: If user loads "legion1.adf" and legion2.adf, legion3.adf exist:
 *   DF0 = legion1.adf, DF1 = legion2.adf, DF2 = legion3.adf
 * If legion0.adf also exists:
 *   DF0 = legion1.adf, DF1 = legion2.adf, DF2 = legion3.adf, DF3 = legion0.adf (autosave last)
 */

/* Parse filename to extract: base name, disk number, extension
 * Returns disk number (1-99), or -1 if not a numbered disk
 * Example: "legion2.adf" -> base="legion", num=2, ext=".adf"
 */
static int parse_multidisk_name(const char* fname, char* base_out, char* ext_out)
{
    int len = strlen(fname);
    if (len < 5) return -1;  /* minimum: "x1.adf" */
    ...
    if (dot_pos < 2) return -1;  /* need at least "x1.ext" */
    ...
    if (num_start == num_end) return -1;  /* no number found */
...
/* Main multi-disk detection and loading function
 * Called when a disk is inserted - checks for other numbered disks
 */
void disk_insert_multidisk(const char* name)
{
    char base[256], ext[32];
    char path_prefix[256];

    int disk_num = parse_multidisk_name(name, base, ext);
    if (disk_num < 0) {
        /* Not a numbered disk, just load normally */
        disk_insert(0, name);
        return;
    }

    /* Find directory prefix from full path */
    int last_sep = -1;
    int len = strlen(name);
    for (int i = 0; i < len; i++) {
        if (name[i] == '/' || name[i] == '\\') {
    ...
    if (last_sep >= 0) {
        strncpy(path_prefix, name, last_sep + 1);
        path_prefix[last_sep + 1] = '\0';
        /* Adjust base to be just filename part */
        char temp_base[256];
        parse_multidisk_name(name + last_sep + 1, temp_base, ext);
        snprintf(base, sizeof(base), "%s%s", path_prefix, temp_base);
    }

    write_log("Multidisk: base='%s' ext='%s' detected_num=%d\n", base, ext, disk_num);

    /* Find all numbered disks from 0-9 */
    char found_disks[10][256];
    int disk_exists[10] = {0};

    for (int i = 0; i <= 9; i++) {
        char try_path[256];
        snprintf(try_path, sizeof(try_path), "%s%d%s", base, i, ext);
        if (file_exists(try_path)) {
            strcpy(found_disks[i], try_path);
            disk_exists[i] = 1;
            write_log("Multidisk: found disk %d: %s\n", i, try_path);
        }
    }

    /* v055: Build sorted list of ALL disks (up to MAX_MULTIDISK) for shuffler */
    multidisk_count = 0;
    multidisk_offset = 0;
    ...
    for (int i = 1; i <= 9 && multidisk_count < MAX_MULTIDISK; i++) {
        if (disk_exists[i]) {
            strncpy(multidisk_paths[multidisk_count], found_disks[i], 255);
            multidisk_paths[multidisk_count][255] = '\0';
            multidisk_count++;
        }
    }

    /* Then add disk 0 (autosave) at the end if exists */
    if (disk_exists[0] && multidisk_count < MAX_MULTIDISK) {
        strncpy(multidisk_paths[multidisk_count], found_disks[0], 255);
        multidisk_paths[multidisk_count][255] = '\0';
        write_log("Multidisk: disk 0 placed at position %d (autosave)\n", multidisk_count);
        multidisk_count++;
    }

    write_log("Multidisk: total %d disks found\n", multidisk_count);

    /* Load up to NUM_DRIVES disks into drives */
    if (multidisk_count > 0) {
        int drives_to_use = (multidisk_count < NUM_DRIVES) ? multidisk_count : NUM_DRIVES;
        for (int i = 0; i < drives_to_use; i++) {
            write_log("Multidisk: loading DF%d = %s\n", i, multidisk_paths[i]);
            disk_insert(i, multidisk_paths[i]);
        }

        /* v055: Disable unused drives to save chip RAM */
        disabled = 0;
        for (int i = drives_to_use; i < 4; i++) {
            disabled |= (1 << i);
            write_log("Multidisk: disabling DF%d to save chip RAM\n", i);
        }
        write_log("Multidisk: disabled mask = 0x%02X\n", disabled);
    } else {
        /* Fallback: just load the original file */
        disk_insert(0, name);
        /* Single disk = disable df1, df2, df3 */
        disabled = 0x0E;  /* bits 1,2,3 = drives df1,df2,df3 disabled */
        multidisk_count = 1;
        strncpy(multidisk_paths[0], name, 255);
        multidisk_paths[0][255] = '\0';
        write_log("Multidisk: single disk, disabled mask = 0x%02X\n", disabled);
    }
}

/* v057: Sync multidisk state from prefs_df[] (for cmdline-loaded disks)
 * Called when multidisk_count is 0 but disks were loaded via -df0/-df1 etc
 * Now also detects 5+ disks by scanning folder for additional numbered disks
 */
static void disk_sync_from_prefs(void)
{
    multidisk_count = 0;
    multidisk_offset = 0;

    /* First sync existing 4 drives from prefs_df */
    for (int i = 0; i < NUM_DRIVES; i++) {
        if (prefs_df[i][0] != '\0') {
            strncpy(multidisk_paths[multidisk_count], prefs_df[i], 255);
            multidisk_paths[multidisk_count][255] = '\0';
            multidisk_count++;
            write_log("disk_sync: DF%d = %s\n", i, prefs_df[i]);
        }
    }
    write_log("disk_sync: synced %d disks from prefs_df\n", multidisk_count);

    /* v057: Try to detect additional disks (5-9) from first disk's pattern */
    if (multidisk_count > 0 && prefs_df[0][0] != '\0') {
        const char* full_path = prefs_df[0];
        int len = strlen(full_path);

        /* Find directory prefix and filename */
        int last_sep = -1;
        for (int i = 0; i < len; i++) {
        ...
        if (last_sep >= 0) {
            strncpy(path_prefix, full_path, last_sep + 1);
            path_prefix[last_sep + 1] = '\0';
            filename = full_path + last_sep + 1;
        }

        /* Parse filename for disk pattern */
        char base[256], ext[32];
        int disk_num = parse_multidisk_name(filename, base, ext);

        if (disk_num >= 0) {
            write_log("disk_sync: detected pattern base='%s' ext='%s' from disk %d\n", base, disk_num, ext);

            /* Build full base path including directory */
            char full_base[256];
            snprintf(full_base, sizeof(full_base), "%s%s", path_prefix, base);

            /* Look for additional disks numbered 5-9 */
            for (int i = 5; i <= 9 && multidisk_count < MAX_MULTIDISK; i++) {
                char try_path[256];
                snprintf(try_path, sizeof(try_path), "%s%d%s", full_base, i, ext);
                if (file_exists(try_path)) {
                    strncpy(multidisk_paths[multidisk_count], try_path, 255);
                    multidisk_paths[multidisk_count][255] = '\0';
                    write_log("disk_sync: found additional disk %d: %s\n", i, try_path);
                    multidisk_count++;
                }
            }
        }
    ...
    /* v057: Update disabled mask - enable only drives that have disks */
    int drives_to_use = (multidisk_count < NUM_DRIVES) ? multidisk_count : NUM_DRIVES;
    disabled = 0;
    for (int i = drives_to_use; i < 4; i++) {
        disabled |= (1 << i);
    }
    write_log("disk_sync: total %d disks, disabled mask = 0x%02X\n", multidisk_count, disabled);
}

/* v057: Shuffle disks - rotate all disks one position forward
 * disk[0] goes to end, disk[1] becomes disk[0], etc.
 * Handles games with more than 4 disks (up to 10)
 * Now also resets disabled mask to enable all active drives
 */
void disk_shuffle(void)
{
    /* Lazy init: sync from prefs_df if needed */
    if (multidisk_count == 0) {
        for (int i = 0; i < NUM_DRIVES; i++) {
            if (prefs_df[i][0] != '\0') {
                disk_sync_from_prefs();
                break;
            }
        }
    }
    ...
    if (multidisk_count <= 1) {
        write_log("Shuffle: only %d disk(s), nothing to shuffle\n", multidisk_count);
        return;
    }

    /* Increment rotation offset */
    multidisk_offset = (multidisk_offset + 1) % multidisk_count;
    write_log("Shuffle: new offset = %d (disk %d now in df0)\n",
              multidisk_offset, multidisk_offset + 1);

    /* Reload drives with rotated disks */
    int drives_to_use = (multidisk_count < NUM_DRIVES) ? multidisk_count : NUM_DRIVES;

    for (int drive = 0; drive < drives_to_use; drive++) {
        int disk_idx = (multidisk_offset + drive) % multidisk_count;

        /* Eject current disk */
        disk_eject(drive);

        /* Insert rotated disk */
        write_log("Shuffle: DF%d = disk %d (%s)\n", drive, disk_idx + 1, multidisk_paths[disk_idx]);
        disk_insert(drive, multidisk_paths[disk_idx]);
    }

    /* v057: Reset disabled mask - enable drives with disks, disable empty ones */
    disabled = 0;
    for (int i = drives_to_use; i < 4; i++) {
        disabled |= (1 << i);
    }
    write_log("Shuffle: disabled mask reset to 0x%02X\n", disabled);
}

/* v055: Get disk info for display in menu
 * Returns pointer to filename part of path, truncated to max_len
 */
const char* disk_get_name(int drive, char* buf, int max_len)
{
    ...
    /* Get actual disk index based on rotation */
    int drives_loaded = (multidisk_count < NUM_DRIVES) ? multidisk_count : NUM_DRIVES;
    if (drive >= drives_loaded || multidisk_count == 0) {
        strncpy(buf, "<empty>", max_len);
        buf[max_len-1] = '\0';
        return buf;
    }

    int disk_idx = (multidisk_offset + drive) % multidisk_count;
    const char* path = multidisk_paths[disk_idx];
...
/* v055: Get total number of disks detected */
int disk_get_multidisk_count(void)
{
    /* Lazy init: if multidisk_count is 0 but disks are loaded, sync from prefs */
    if (multidisk_count == 0) {
        /* Check if any disk is actually loaded */
        for (int i = 0; i < NUM_DRIVES; i++) {
            if (prefs_df[i][0] != '\0') {
                disk_sync_from_prefs();
                break;
